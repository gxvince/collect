...
export class PageConfigService {
  ...
  async upsertAttachData({ siteId, pageId, attachData }) {
    await this.dbPool.query(
      `INSERT INTO site_page_configs (site_id, page_id, materials_json, sizes_json, attach_data_json)
       VALUES (?, ?, NULL, NULL, ?)
       ON DUPLICATE KEY UPDATE attach_data_json = VALUES(attach_data_json), updated_at = CURRENT_TIMESTAMP`,
      [siteId, pageId, JSON.stringify(attachData || [])],
    );
  }

  async findBySitePage(siteId, pageId) {
    const [rows] = await this.dbPool.query(
      `SELECT id, site_id, page_id, materials_json, sizes_json, attach_data_json, created_at, updated_at
