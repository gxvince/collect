...
export class AuthService {
  ...
  async getUserSitePagesWithNames(user) {
    if (user.role === 'admin') {
      return [];
    }

    const [rows] = await this.dbPool.query(
      `SELECT usp.site_id, usp.page_id, usp.allow, s.wp_base_url, s.wp_auth_token
       FROM user_site_pages usp
       INNER JOIN user_sites us ON us.user_id = usp.user_id AND us.site_id = usp.site_id
       INNER JOIN sites s ON s.site_id = usp.site_id AND s.is_deleted = 0
       WHERE usp.user_id = ?
    ...
    rows.forEach((row) => {
      ...
      if (!siteMap.has(siteId)) {
        siteMap.set(siteId, {
          site_id: siteId,
          wp_base_url: row.wp_base_url ? String(row.wp_base_url) : '',
          wp_auth_token: row.wp_auth_token ? String(row.wp_auth_token) : '',
          pages: [],
        });
      }
    ...
    const pageNameMaps = await Promise.all(
      sites.map(async (site) => ({
        site_id: site.site_id,
        map: await this.fetchSitePageNameMap(
          site.wp_base_url,
          site.wp_auth_token,
        ),
      })),
    );
    const nameMapBySite = new Map(
      pageNameMaps.map((item) => [item.site_id, item.map]),
    );

    return sites.map((site) => {
      const nameMap = nameMapBySite.get(site.site_id) || new Map();
      return {
        site_id: site.site_id,
        pages: site.pages.map((item) => ({
          page_id: item.page_id,
          page_name: nameMap.get(item.page_id) || '',
          allow: item.allow,
        })),
      };
    });
  ...
  async fetchSitePageNameMap(baseUrl, authToken) {
    const map = new Map();
    ...
    const url = `${base}/wp-json/custom-db-api/v1/publish_pages`;
    try {
      ...
      list.forEach((item) => {
        const pageId = Number(item.ID ?? item.id);
        if (!Number.isFinite(pageId) || pageId <= 0) {
          return;
        }
        const pageName = item.post_name ? String(item.post_name) : '';
        map.set(pageId, pageName);
      });
    } catch (error) {
      return map;
    }
